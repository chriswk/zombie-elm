...
Elm.Components.Zombie = Elm.Components.Zombie || {};
Elm.Components.Zombie.make = function (_elm) {
   "use strict";
   _elm.Components = _elm.Components || {};
   _elm.Components.Zombie = _elm.Components.Zombie || {};
   if (_elm.Components.Zombie.values)
   return _elm.Components.Zombie.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   var tileHtml = function (tile) {
      var peeking = tile.peeking ? " peeking" : "";
      var matched = tile.matched ? " match" : "";
      var revealed = tile.revealed ? " revealed" : "";
      var tileClass = A2($Basics._op["++"],
      "tile",
      A2($Basics._op["++"],revealed,matched));
      var backClassname = A2($Basics._op["++"],"back ",tile.name);
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("cell")]),
      _U.list([A2($Html.div,
      _U.list([$Html$Attributes.$class(tileClass)]),
      _U.list([A2($Html.div,
              _U.list([$Html$Attributes.$class("front")]),
              _U.list([]))
              ,A2($Html.div,
              _U.list([$Html$Attributes.$class(backClassname)]),
              _U.list([]))]))]));
   };
   var lineHtml = function (tileList) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("line")]),
      A2($List.map,tileHtml,tileList));
   };
   var view = F2(function (address,model) {
      return lineHtml(model.tiles);
   });
   var Model = F2(function (a,b) {    return {seed: a,tiles: b};});
   var Tile = F4(function (a,b,c,d) {
      return {revealed: a,peeking: b,matched: c,name: d};
   });
   var newTile = function (name) {
      return A4(Tile,false,false,false,name);
   };
   var gameTiles = A2($List.map,
   newTile,
   _U.list(["h1","h1","h2","h2","h3","h3","h4","h4","h5","h5"]));
   var NoOp = {ctor: "NoOp"};
   return _elm.Components.Zombie.values = {_op: _op
                                          ,NoOp: NoOp
                                          ,Tile: Tile
                                          ,Model: Model
                                          ,newTile: newTile
...
Elm.Main.make = function (_elm) {
   ...
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Components$Zombie = Elm.Components.Zombie.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Effects = Elm.Effects.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var initialModel = {tiles: _U.list([])
                      ,seed: $Random.initialSeed(5152)};
   var init = function () {
      var tiles = $Components$Zombie.gameTiles;
      var m = _U.update(initialModel,{tiles: tiles});
      return {ctor: "_Tuple2",_0: m,_1: $Effects.none};
   }();
   var app = $StartApp.start({init: init
                             ,inputs: _U.list([])
                             ,update: $Components$Zombie.update
                             ,view: $Components$Zombie.view});
   var main = app.html;
